...
- Baku 2024's fitted MEDIUM tire wear slope came out negative, unlike the corrected Bahrain and Monaco models. This was investigated directly: the negative slope isn't caused by fuel effect (already handled) or by soft tire contamination (ruled out separately, see below). Comparing tire age against actual lap number showed the real cause: several drivers started the race on medium tires, so their `TyreLife == 1` lap coincides with `LapNumber == 1`, the opening lap of the race. Opening laps are slower than normal racing laps regardless of tire compound, due to the standing start and bunched field through the first corner, and this anchors the low end of the tire-age fit at an artificially slow point, pulling the slope negative. Cross-referencing against Baku's actual caution periods (which occurred around laps 36 and 49-51) ruled out safety car restart contamination as the cause. Not yet fixed: a corrected version would need to exclude lap 1 specifically from the degradation fit, separate from the existing pit in/out and caution filters.
